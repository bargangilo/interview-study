...
- **Problem config & detection:** `problems/<name>/` (read-only, never written at runtime)
- **Working files:** `workspace/<name>/main.js` or `main.py` (created/written by CLI)
- **Tests:** `tests/<name>/` (import from `workspace/`, not `problems/`)
...
3. Create `tests/<name>/sample.test.js` (Jest) importing from `../../workspace/<name>/main`
4. Create `tests/<name>/test_sample.py` (pytest) with `sys.path` pointing to `../../workspace/<name>`
